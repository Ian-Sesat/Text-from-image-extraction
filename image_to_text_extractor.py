...
def detect_black_boxes(pdf_path):
    """
    Detect black boxes within the PDF.
    """
    black_boxes = []
...
def main():
    ...
    pdf_path = 'test PDFann.pdf'
    
    # Detect black boxes within the PDF and extract text from them
    black_boxes = detect_black_boxes(pdf_path)
    count=0
    dwg_number = None
    if black_boxes:
        print("Text extracted from black boxes:")
        for text, bbox in black_boxes:
            #print("Box:", bbox)
            print("Text:", text)
            print("---")
            count=count+1;
        # Extract DWG number from the first page
        with fitz.open(pdf_path) as pdf_file:
        ...
        print("No black boxes found in the PDF.")
    print('Count :' ,count)
